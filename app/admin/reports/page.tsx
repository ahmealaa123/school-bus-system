...
import { FaBus, FaCalendarAlt, FaFilter, FaFileExcel, FaFilePdf, FaCheckCircle, FaTimesCircle } from "react-icons/fa";
...
export default function ReportsPage() {
  ...
  const exportPDF = async () => {
    try {
      ...
      autoTable(doc, {
        startY: 35,
        head: [["الباص", "المشرفة", "الرحلة", "الحالة", "عدد الحضور", "عدد الغياب", "الأسماء", "التاريخ"]],
        body: filtered.map((r) => [
          r.bus,
          r.supervisorName,
          r.tripId,
        ...
        theme: "grid",
        headStyles: { fillColor: [79, 70, 229], textColor: [255, 255, 255], font: "Amiri", fontStyle: "normal", halign: "right" },
        bodyStyles: { font: "Amiri", fontStyle: "normal", halign: "right", valign: "middle" },
        alternateRowStyles: { fillColor: [243, 244, 246] },
        styles: { fontSize: 11, cellPadding: 4, overflow: "linebreak", lineWidth: 0.1, lineColor: [0, 0, 0] },
        didParseCell: (data) => {
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div className="flex flex-col">
              ...
              <select value={busFilter} onChange={(e) => setBusFilter(e.target.value)} className="bg-gray-700 p-3 rounded-lg border border-gray-600 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/50 transition-all">
            ...
            <div className="flex flex-col">
              ...
              <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="bg-gray-700 p-3 rounded-lg border border-gray-600 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/50 transition-all">
            ...
            <div className="flex flex-col">
              ...
              <input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} className="bg-gray-700 p-3 rounded-lg border border-gray-600 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/50 transition-all" />
            </div>
            ...
            <div className="flex flex-col">
              ...
              <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} className="bg-gray-700 p-3 rounded-lg border border-gray-600 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/50 transition-all" />
            </div>

            <div className="flex items-end gap-3">
              <motion.button whileTap={{ scale: 0.95 }} onClick={applyFilter} className="flex-1 bg-indigo-600 hover:bg-indigo-700 p-3 rounded-lg font-bold shadow-lg transition-all">
                تطبيق الفلتر
              </motion.button>
            </div>
          </div>
          ...
          <div className="flex gap-4 mt-6">
            <motion.button whileTap={{ scale: 0.95 }} onClick={exportExcel} className="flex items-center gap-2 bg-green-600 hover:bg-green-700 p-3 rounded-lg font-bold shadow-lg transition-all">
              <FaFileExcel /> Excel
            </motion.button>

            <motion.button whileTap={{ scale: 0.95 }} onClick={exportPDF} className="flex items-center gap-2 bg-red-600 hover:bg-red-700 p-3 rounded-lg font-bold shadow-lg transition-all">
              <FaFilePdf /> PDF
            </motion.button>
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="glass overflow-x-auto rounded-3xl shadow-2xl border border-gray-700">
            ...
            <tbody>
              {filtered.map((r, i) => (
                ...
                >
                  ...
                  <td className="p-4">{r.supervisorName}</td>
                  ...
                  <td className="p-4">
                    <motion.button whileTap={{ scale: 0.95 }} onClick={() => openModal(r)} className="bg-indigo-600 hover:bg-indigo-700 px-6 py-2 rounded-lg text-sm font-medium transition-all">
                      عرض التفاصيل
                    </motion.button>
                  </td>
                </motion.tr>
                  ...
                  <Dialog.Panel className="w-full max-w-3xl transform overflow-hidden rounded-2xl bg-gray-900 p-8 text-left align-middle shadow-xl transition-all border border-indigo-500/30">
                    ...
                    <div className="mt-2 space-y-6">
                      <div className="grid grid-cols-2 gap-4 text-gray-300">
                        ...
                        <div>
                          <p className="font-semibold">المشرفة:</p>
                          <p>{selectedTrip?.supervisorName}</p>
                        </div>
                        <div>
